...
define("CONTROLLERS_ADMINS", "../controllers/admin.php");
...
class handle extends database
{
    ...
    {
        $sql = "INSERT INTO `history`(`id_user`, `id_order`) VALUES ('$id_user','$id_order')";
        $this->execute($sql);
    }
    public function get_histrory($id_user)
    {
        $sql = "SELECT * FROM `history` JOIN users ON users.id_user = history.id_user
        JOIN orders ON orders.id_order = history.id_order WHERE users.id_user = $id_user";
        $gan = $this->select($sql);
        return $gan;
    }
